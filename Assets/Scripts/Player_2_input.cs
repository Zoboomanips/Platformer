    ...
    void Update()
    {
        ...
        if (!stunned)
        {
            if (control)
            {
                ...
                if (Input.GetAxis("Horizontal2") > 0 || Input.GetAxis("Horizontal2") < 0)
                {
                ...
                if (chara.GetComponent<Rigidbody2D>().velocity.y == 0 && !(Input.GetAxis("Horizontal2") < 0 || Input.GetAxis("Horizontal2") > 0 || Input.GetKey(KeyCode.Joystick2Button2)))
                {
                    CharAni.SetTrigger("Idle");
                }
